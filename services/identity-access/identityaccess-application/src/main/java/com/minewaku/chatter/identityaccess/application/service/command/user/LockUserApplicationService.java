...
public class LockUserApplicationService implements LockUserUseCase {
	...
	public Void handle(UserId userId) {
		...
		UserLockedIntegrationEvent event = new UserLockedIntegrationEvent(user.getId().getValue().toString());
		IntegrationEventWrapper<UserLockedIntegrationEvent> wrapper = new IntegrationEventWrapper<>(eventId, user.getId().getValue().toString(), event);
