...
public class RedisConfirmationTokenRepository {
    ...
    private final RedisTemplate<String, ConfirmationTokenCacheDto> redisTemplateConfirmationToken;
    private final RedisTemplate<String, String> redisTemplateString;

    public RedisConfirmationTokenRepository(
            RedisTemplate<String, ConfirmationTokenCacheDto> redisTemplateConfirmationToken,
    ...
    public Optional<ConfirmationTokenCacheDto> findByToken(String token) {
        String key = Prefix.CONFIRMATION_TOKEN_VALUE.format(token);

        ConfirmationTokenCacheDto confirmationTokenDto = redisTemplateConfirmationToken.opsForValue().get(key);
        return Optional.ofNullable(confirmationTokenDto);
    }


    public ConfirmationTokenCacheDto save(ConfirmationTokenCacheDto confirmationTokenDto) {
    ...
    public void deleteByToken(String token) {
        ...
        ConfirmationTokenCacheDto dto = redisTemplateConfirmationToken.opsForValue().get(valueKey);
        if (dto != null) {
            String lookupKey = Prefix.CONFIRMATION_TOKEN_LOOKUP.format(dto.email());
            redisTemplateString.delete(lookupKey);
            redisTemplateConfirmationToken.delete(valueKey);
        }
    }
